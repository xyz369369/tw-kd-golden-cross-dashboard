...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT date, market, code, name, open, high, low, close, volume FROM daily_prices ORDER BY code, date", conn
    )
    conn.close()
    return df
...
def screen():
    ...
    for code, g in df.groupby("code"):
        ...
        if cond_price and cond_ma_price and cond_ma_vol and golden_cross:
            results.append({
                "code": code,
                "name": last["name"],
                "market": last["market"],
    ...
    output = {
        ...
        "criteria": "股價<150, 收盤價>MA20, 成交量>量MA20, KD(9,3,3)黃金交叉",
